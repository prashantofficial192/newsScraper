...
const url = 'https://www.moneycontrol.com/news/business/mutual-funds/';

// Create model for "mutualFundNews" collection
const MutualFundNews = mongoose.models.MutualFundNews || mongoose.model('MutualFundNews', newsSchema, 'mutualFundNews');
...
export async function getMutualFundsNews() {
    try {
        // Step 1: Connect to DB
        await connectToDatabase();

        // Step 2: Send message to Discord before scraping economy news
        await sendDiscordMessage('6. Scraping Mutual Funds News...');
        ...
        for (const article of fullNewsData) {
            const exists = await MutualFundNews.exists({ link: article.link });
            if (!exists) {
        ...
        if (bulkInserts.length > 0) {
            await MutualFundNews.insertMany(bulkInserts);
            await sendDiscordMessage(`6. Inserted ${bulkInserts.length} new articles into the database.`);
            // console.log(`✅ Inserted ${bulkInserts.length} new articles`);
        } else {
            // console.log("ℹ️ All articles already exist. Nothing new to insert.");
            await sendDiscordMessage('ℹ️ All articles already exist. Nothing new to insert in mutualFundNews collection')
        }


        // Step 8: Send message to Discord after scraping is completed
        await sendDiscordMessage(`6. Mutual Funds News Scraping completed with ${fullNewsData.length} articles.`);
