...
// Create model for  "commoditiesNews" collection
const CommoditiesNews = mongoose.models.CommoditiesNews || mongoose.model('CommoditiesNews', newsSchema, 'commoditiesNews');
...
export async function getCommoditiesNews() {
    try {
        ...
        await sendDiscordMessage('7. Scraping Commodities News...');
        ...
        for (const article of fullNewsData) {
            const exists = await CommoditiesNews.exists({ link: article.link });
            if (!exists) {
        ...
        if (bulkInserts.length > 0) {
            await CommoditiesNews.insertMany(bulkInserts);
            await sendDiscordMessage(`7. Inserted ${bulkInserts.length} new articles into the database.`);
            // console.log(`✅ Inserted ${bulkInserts.length} new articles`);
        } else {
            // console.log("ℹ️ All articles already exist. Nothing new to insert.");
            await sendDiscordMessage('ℹ️ All articles already exist. Nothing new to insert in commoditiesNews collection')
        }


        // Step 8: Send message to Discord after scraping is completed
        await sendDiscordMessage(`7. Commodities News Scraping completed with ${fullNewsData.length} articles.`);
