...
// Create model for "economyNews" collection
const EconomyNews = mongoose.models.EconomyNews || mongoose.model('EconomyNews', newsSchema, 'economyNews');
...
// Main function to get latest economy news from MoneyControl
export async function getEconomyNews() {
    try {
        ...
        await sendDiscordMessage('4. Scraping Economy News...');
        ...
        for (const article of fullNewsData) {
            const exists = await EconomyNews.exists({ link: article.link });
            if (!exists) {
        ...
        if (bulkInserts.length > 0) {
            await EconomyNews.insertMany(bulkInserts);
            await sendDiscordMessage(`4. Inserted ${bulkInserts.length} new articles into the database.`);
            // console.log(`✅ Inserted ${bulkInserts.length} new articles`);
        } else {
            // console.log("ℹ️ All articles already exist. Nothing new to insert.");
            await sendDiscordMessage('ℹ️ All articles already exist. Nothing new to insert in economyNews collection')
        }


        // Step 8: Send message to Discord after scraping is completed
        await sendDiscordMessage(`4. Economy News Scraping completed with ${fullNewsData.length} articles.`);
