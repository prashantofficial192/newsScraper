...
// Create model for "ipoNews" collection
const IpoNews = mongoose.models.IpoNews || mongoose.model('IpoNews', newsSchema, 'ipoNews');
...
export async function getIpoNews() {
    try {
        ...
        await sendDiscordMessage('5. Scraping IPO News...');
        ...
        for (const article of fullNewsData) {
            const exists = await IpoNews.exists({ link: article.link });
            if (!exists) {
        ...
        if (bulkInserts.length > 0) {
            await IpoNews.insertMany(bulkInserts);
            await sendDiscordMessage(`5. Inserted ${bulkInserts.length} new articles into the database.`);
            // console.log(`✅ Inserted ${bulkInserts.length} new articles`);
        } else {
            // console.log("ℹ️ All articles already exist. Nothing new to insert.");
            await sendDiscordMessage('ℹ️ All articles already exist. Nothing new to insert in IpoNews collection')
        }


        // Step 8: Send message to Discord after scraping is completed
        await sendDiscordMessage(`5. IPO News Scraping completed with ${fullNewsData.length} articles.`);
